...
class MessageClient(object):
    ...
    def __init__(self):
        ...
        self.stub = pb2_grpc.BotMessageStub(self.channel)
...
class MenuClient(object):
    ...
    def __init__(self):
        ...
        # bind the client and the server
        self.stub = pb2_grpc.BotMenuStub(self.channel)
...
async def buttons_from_db(tag: str, client_id: int) -> Optional[list]:

    client = MenuClient()
    result = client.get_menu_from_db(tag, client_id)
    print(f'{result=}')
    return result
...
async def start(request):
    """Call pong"""
    logging.info('pong')
    result = await buttons_from_db('tag', 123)
